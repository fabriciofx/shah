# Agent Guidelines for Java Code Quality
...
The README.md file must be as short as possible and must not duplicate code
documentation.

Every class must have a supplementary docblock preceding it.
A class docblock must explain the purpose of the class and provide usage
examples.
...
The principle of "Paired Brackets" suggested by Yegor Bugayenko must be
respected.
...
Implementation inheritance must be avoided at all costs (not to be confused with
subtyping).
...
Every class may have only one primary constructor; any secondary constructor
must delegate to it.
...
Method names must respect the CQRS principle: they must be either nouns or
verbs.
...
Every assertion must include a failure message that is a negatively toned claim
about the error.
...
Tests must be named as full English sentences, stating what the object under
test does.
Tests may not test functionality irrelevant to their stated purpose.
Tests must close resources they use, such as file handlers, sockets, and
...
Tests must not clean up after themselves; instead, they must prepare a clean
state at the start.
...
Each test must verify only one specific behavioral pattern of the object it
tests.
...
The testing framework must be configured to disable logging from the objects
under test.
Tests must not wait indefinitely for any event; they must always stop waiting on
a timeout.
...
Tests must use ephemeral TCP ports, generated using appropriate library
functions.
